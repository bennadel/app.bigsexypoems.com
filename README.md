...
* **`build`**: Client-side (JavaScript + LessCSS) compilation mechanics. This folder has its own `docker-compose.yaml` and `Dockerfile` files.
...
* **`wwwroot`**: Publicly accessible CFML files. The `index.cfm` file simply includes `/client/index.cfm`.
  ...
  * **`static/main`**: `.cfm`, `.js`, and `.css` files generated by the Parcel.js build system (a custom reporter wraps each entry's `<link>`/`<script>` tags into a CFML module), consumed by ColdFusion layout templates via `cfmodule`.
...
The client-side assets are managed in the `cfml/app/build/main` folder. This folder has its own `docker-compose.yaml` file and `Dockerfile` for `node`. All `npm` commands must be run inside the Docker `app` container &mdash; never directly on the host machine.

From within the `cfml/app/build/main` folder:
...
1. Shell into the app with `docker compose run --rm app sh`.
2. Execute `npm` commands to modify the `package*.json` files.
3. Run `exit` to quit and remove the temporary container.
