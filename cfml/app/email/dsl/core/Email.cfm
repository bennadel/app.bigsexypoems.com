...
<cfswitch expression="#thistag.executionMode#">
	...
	<cfcase value="end">
		<cfoutput>
				...
				<head>
					...
					<cfif arrayLen( theme.importUrls )>
						...
						<cfmodule template="./IfNotMso.cfm">
							...
							<cfloop index="importUrl" array="#theme.importUrls#">
								<style type="text/css">
			...
			--->
			<cfset thistag.generatedContent = minifyEmailContent( fixProtocolEncoding( emailContent ) ) />
...
<cfscript>
	...
	/**
	* Some email clients (YahooMail) don't like it when link protocols are encoded for
	* HTML attributes. Since this is perfectly valid HTML, I want to see if I can fix
	* this at a higher-level rather than pushing the responsibility down to the low-
	* level components or the developers.
	*/
	public string function fixProtocolEncoding( required string content ) {

		// Calculate the encoded value.
		var unecodedSuffix = "://";
		var encodedSuffix = encodeForHtmlAttribute( unecodedSuffix );

		// Fix HREF and SRC attributes.
		var fixedContent = variables.reReplaceAll(
			arguments.content,
			"(?i)((href|src)\s*=\s*['""]https?)#encodedSuffix#",
			"$1#unecodedSuffix#"
		);
